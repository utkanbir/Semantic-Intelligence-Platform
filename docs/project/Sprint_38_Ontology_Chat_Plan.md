...
| S38-04 | Contract documentation: add `POST /api/v1/chat/ontology` to `docs/architecture/SIP_Ontology_Definition_Contract_v1.md` (new `§8.7 Ontology chat` table, same `| Method | Path | Behavior |` format as §8.6) *before or in the same PR as* S38-05, so contract-sync CI has something to match against | Docs | No | — | #384 |
