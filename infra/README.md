...
| **Docker** | Build `sip-backend:s14` and `sip-console:s21` images | Required for in-cluster workloads |
...
docker build -t sip-console:s21 frontend
...
The dev overlay pins **`sip-console:s21`** and **`sip-backend:s14`** (see `infra/kubernetes/overlays/dev/kustomization.yaml` `images` section). Rebuild and re-apply after frontend or backend changes:
