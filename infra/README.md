...
| **Docker** | Build `sip-backend:s14` and `sip-console:s20` images | Required for in-cluster workloads |
...
docker build -t sip-console:s20 frontend
...
The dev overlay pins **`sip-console:s20`** and **`sip-backend:s14`** (see `infra/kubernetes/overlays/dev/kustomization.yaml` `images` section). Rebuild and re-apply after frontend or backend changes:
