...
| **Docker** | Build sprint-tagged `sip-backend:s<N>` and `sip-console:s<N>` images | Required for in-cluster workloads |
...
docker build -t sip-backend:s<N> backend
docker build -t sip-console:s<N> frontend
...
The dev overlay pins versioned `sip-console` and `sip-backend` images in `infra/kubernetes/overlays/dev/kustomization.yaml`. Rebuild the exact tags pinned there and re-apply after frontend or backend changes:
...
### Sprint-close deploy verification

Before PMO declares the sprint complete, verify that the dev overlay pins the sprint's expected images and that both live `sip-dev` deployments have rolled out those exact tags:

```powershell
powershell -File scripts/verify-sprint-deploy.ps1 -Sprint <N>
```

Expectations per sprint: `scripts/sprint_deploy_expectations.json`. Starting with Sprint 31, missing or stale image expectations are sprint-close blockers.
