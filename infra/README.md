...
| **Docker** | Build `sip-backend:s14` and `sip-console:s15` images | Required for in-cluster workloads |
...
docker build -t sip-console:s15 frontend
...
The dev overlay pins **`sip-console:s15`** and **`sip-backend:s14`** (see `infra/kubernetes/overlays/dev/kustomization.yaml` `images` section). Rebuild and re-apply after frontend or backend changes:
