...
| **Docker** | Build `sip-backend:s14` and `sip-console:s17` images | Required for in-cluster workloads |
...
docker build -t sip-console:s17 frontend
...
The dev overlay pins **`sip-console:s17`** and **`sip-backend:s14`** (see `infra/kubernetes/overlays/dev/kustomization.yaml` `images` section). Rebuild and re-apply after frontend or backend changes:
